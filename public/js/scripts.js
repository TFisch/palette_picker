...
retrieveSavedPalettes = async () => {
  try {
    ...
    const response = await fetch(url);
    const data = await response.json();
    const match = data.map(palette =>
      $(`#proj${palette.project_id}`).append(`
...
$('.color').click(function(event) {
  let imageState = $(event.target)
    .children('img')
    .attr('src');
  if (imageState === './images/unlocked.svg') {
    $(event.target)
      .children('img')
      .attr('src', './images/locked.svg');
...
$(function() {
  ...
  $('.lock-image').click(function(e) {
    ...
    if (imageState === './images/unlocked.svg') {
      ...
      $(event.target)
        .parent('div')
        .toggleClass('unlocked');
      $(event.target)
        .parent('div')
        .toggleClass('locked-in');
    }
    if (imageState === './images/locked.svg') {
